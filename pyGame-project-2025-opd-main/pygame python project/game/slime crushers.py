...
def game_loop(map_type):
    ...
    while running:
        ...
        if player1.disable_controls and current_time - player1.disable_controls_timer > PUSH_DISABLE_DURATION:
            player1.disable_controls = False
            player1.is_pushed = False

        if player2.disable_controls and current_time - player2.disable_controls_timer > PUSH_DISABLE_DURATION:
            player2.disable_controls = False
            player2.is_pushed = False
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                # Событие возврата управления после толчка
                player1.disable_controls = False
                player1.is_pushed = False
                player2.disable_controls = False
                player2.is_pushed = False
        ...
        if not game_paused and not show_countdown:
            ...
            if not player1.disable_controls:
                ...
                if keys[pygame.K_1] and player1.has_gauntlet:  # Кнопка 1 для перчатки
                    player1.push_enemy(player2)

            if not player2.disable_controls:
                ...
                if keys[pygame.K_p] and player2.has_gauntlet:  # Кнопка p для перчатки
                    player2.push_enemy(player1)
            ...
            if not game_paused:
                player1.update_push_effect()
                player2.update_push_effect()

                # Проверка возврата управления
                current_time = pygame.time.get_ticks()
                for player in [player1, player2]:
                    if player.disable_controls and current_time - player.disable_controls_timer > PUSH_DISABLE_DURATION:
                        player.disable_controls = False
                        player.is_pushed = False  # На всякий случай
