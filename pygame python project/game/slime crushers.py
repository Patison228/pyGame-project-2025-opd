...
menu_background = load_background(
    "C:/Users/Denis/Desktop/pyGame-project-2025-opd-main/pygame python project/image/main_menu_bg.png")
sky_background = load_background(
    "C:/Users/Denis/Desktop/pyGame-project-2025-opd-main/pygame python project/image/bg_sky.jpg")
...
def game_loop(map_type):
    ...
    musicManager.play_game()  # Включаем игровую музыку
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    elif musicManager.current_music in musicManager.game_music_list:
                        musicManager.next_game_track()
...
def select_map():
    ...
    # Получаем текущее состояние музыки перед любыми изменениями
    music_type, music_index = musicManager.get_current_track_info()
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                for i, button in enumerate(buttons):
                    if button.rect.collidepoint(mouse_pos):
                        if i == 3:  # Кнопка Back
                            # Восстанавливаем предыдущую музыку
                            if music_type == "menu":
                                musicManager.current_menu_index = music_index
                                musicManager.play_menu()
                            elif music_type == "game":
                                musicManager.current_game_index = music_index
...
def start_game():
    # Сохраняем текущее состояние музыки меню
    global menu_music_index
    if musicManager.current_music in musicManager.menu_music_list:
        menu_music_index = musicManager.current_menu_index

    map_choice = select_map()
    if map_choice == "menu":
        # Восстанавливаем музыку меню
        musicManager.current_menu_index = menu_music_index
        musicManager.play_menu()
        return main_menu()
    else:
        # Запускаем игру с сохранением текущего индекса
        result = game_loop(map_choice)
        # После возврата из игры восстанавливаем музыку меню
        if result == "menu":
            musicManager.current_menu_index = menu_music_index
            musicManager.play_menu()
        return result
...
def main_menu():
    clock = pygame.time.Clock()
    musicManager.play_menu()  # Включаем меню-музыку при входе в меню
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_b:  # Обработка нажатия B в меню
                    musicManager.next_menu_track()
            for button in buttons:
